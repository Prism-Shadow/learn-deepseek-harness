...
L1 里，bash 工具、安全检查、循环逻辑**全焊死在一起**（病根 ❶）。想加一个 `read_file` 工具？得改 `agentLoop`。想加个日志？还得改 `agentLoop`。循环越长越乱，谁都不敢碰。
...
| 📜 P2 Session Log | 还没上。`history` 仍是可变数组（病根 ❷ 还在）——L3 治。 |
...
插件框架有了，但对话状态还是那个"谁都能乱改、崩了就没"的可变数组 `history`（病根 ❷）。**L3** 上第二根支柱——把 `history` 换成 **append-only 的事件日志**，让「Session Log 成为永远的唯一真相」，模型看到的对话从日志**派生**出来。这是压缩、记忆、崩溃恢复的共同地基。
