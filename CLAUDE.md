...
> **本文件是项目宪法**，每次在本项目干活前先读一遍，所有决策以此为准。
...
- **🧩 P1 一切皆插件**：循环极瘦，只「跑 + 到点喊人」；能力全是挂在 `ctx` + 事件总线上的插件。加功能=加插件，主干不改。
...
L1-L4 已完成并 typecheck 通过；L5 起等用户复盘后推进。

- [x] **L1** 最裸的循环（铺垫；故意留下三处局限 ❶焊死 ❷可变数组 ❸每步重发）
...
- [ ] **L5** 工具管线 + 权限（`pre-execute→guard→execute→post-execute→result`）
- [ ] **L6** 上下文压缩（三支柱合体：插件 + surface replace + 缓存悬崖&前缀复刻）
- [ ] **L7** 跨 session 记忆（召回 + 写入）
- [ ] **L8** 组装成小 harness
- [ ] **L9** 子 agent
- [ ] **L10** 自我修改（一切皆插件的终极）
